...
namespace Osmp.StandardCmds
{
    ...
    public class MemoryCommand : AbstractCommand
    {
        ...
        public override IEnumerable<string> Aliases { get { yield return "mem"; } }

        public override string Description { get { return "Return used memory."; } }

        public override IEnumerable<CommandParam> MandatoryParamDefs { get { yield break; } }

        public override IEnumerable<CommandParam> OptionalParamDefs { get { yield break; } }

        public override IEnumerable<CommandParam> ReturnValueDefs { get { yield return new CommandParam() { Name = "bytes", Description = "Number of bytes currently in use. (long)" }; } }

        protected override async Task<CommandResult> ExecuteImplementation()
        {
            var total = GC.GetTotalMemory(forceFullCollection: true);
            //var text = string.Format("Used memory: {0} ({1})", Conversion.ToHumanReadableBytes(total), total);
            return CommandResult.Success( data:new JObject() { { "bytes", total } });
        }
    }
}
