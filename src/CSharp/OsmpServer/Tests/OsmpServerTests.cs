...
namespace Osmp.Tests
{
    ...
    public class OsmpServerTests
    {
        ...
        {
            using (var server = new OsmpServer() { Port = 44332 })
            using (var client = new OsmpClient() { Address = "wss://localhost:44332/osmp/v1"})
            {
                server.Startup();
        ...
        {
            using (var server = new OsmpServer() { Port = 44332, IsAuthenticationRequired = true, UserCredentials = new Dictionary<string, string>{{"ned", "stark"}}})
            using (var client = new OsmpClient() { Address = "wss://localhost:44332/osmp/v1" })
            {
                server.Startup();
                client.Error += (s, e) => Assert.Fail(s + "\n" + e.PrettyPrint());
        ...
        {
            using (var server = new OsmpServer() { Port = 44332, IsAuthenticationRequired = true, })
            using (var client = new OsmpClient() { Address = "wss://localhost:44332/osmp/v1" })
            {
